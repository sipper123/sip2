...
bool   g_run_rtp        = false;   //if enabled, audio fd will be added, read/write on audio will happen.
...
{
  if (0==pthread_create(&g_au_thread, NULL, CaptureAudioAndSend, (void*)NULL))
  {
...
int ProcessSipMsg(UCHAR* rxStr, char* result)
{
  ...
  if (g_sip_mode == SIP_CLIENT)
  {
          ...
          case 2:
            {
              ...
              g_run_rtp = true;
              strcpy(result, "Remote user has accepted your invite");
            }
            break;
          case 3:
            strcpy(result, msgExtra);
    ...
    if (g_sip_state==SIP_DISCONX_PENDING)
    {
      if (msgType==MSG_INVITE)
      {
        sprintf((char*)g_txBuffer, RESPONSE_MSG, 486, "Busy", g_SessionData.remoteRestHdr, "\0");
...
int main(int argc, char* argv[])
{
  ...
  //@@while (!g_sip_done)
  g_run_rtp=true;
  oSocRTP.Initialize(argv[1], 5009, 5009);
  while (1)
  {
    int  max_fd=0;
    bool local_run_rtp=false;     //i want to ensure rtp state is set at beginning of loop.
    local_run_rtp = g_run_rtp;
    if (local_run_rtp==false)
       g_1stSampleDone = false;   //do this so that the next time rtp starts, first sample will be taken.

    FD_ZERO(&fds);
    //@@FD_SET(0, &fds);
    //@@FD_SET(oSocSIP.m_socket, &fds);
    //@@max_fd = oSocSIP.m_socket;
    if (local_run_rtp)
    {
       FD_SET(g_audio_fd, &fds);
       FD_SET(oSocRTP.m_socket, &fds);
       max_fd = getmaxfd(max_fd, oSocRTP.m_socket, g_audio_fd);
    }

    ret = select(1+max_fd, &fds, NULL, NULL, NULL);
    ...
    */
    printf("local_run_rtp = true\n");
    if (local_run_rtp)
    {
      if (FD_ISSET(oSocRTP.m_socket, &fds))
      {
        if (ReceiveRTPBufferAndPlay()<0) continue;
      }
   
      if (FD_ISSET(g_audio_fd, &fds))
      {
